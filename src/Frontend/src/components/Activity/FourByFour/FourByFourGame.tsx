...
export default function FourByFourGame({ onEndGame, cards }: FourByFourGameProps) {
  ...
  const [description, setDescription] = useState<React.ReactNode>('Объедини 4 тематические карточки');
        ...
        {gameCards?.map(({ id, Icon }) => {
          ...
          return (
            <div
              ...
              <Icon className={clsx(s['game__card-icon'], isIconVisible && s['game__card-icon--visible'])} />
            </div>
          );
        })}
      </div>
